 ...
 <div class="wrap-content-dialog">
      <form id="role-form-add" >
		...
		<tr>
			...
			<td >
			<input  type="text" class="input30 require" length="1,32" label-name="角色名称" name="objBean.roleName"/>
			</td>
			<td class="td-col1" style="width:50px">单位</td>
				<td>
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" class="input30 " style="width: 80%;" length="1,100" label-name="角色描述" name="objBean.roleDes"/>
			</td>
			<td class="text-center">
				<input id="role-add-btn" type="button" class="group_btn" value="保存"/>
			</td>
		</tr>
	 </table>
      </form>
    ...
    <div id="orgContent" class="orgContent" style="position: absolute;  display: none;">
			<ul id="ztree_role_add" class="ztree myztree" style="margin-top:0; ">
			</ul>
	</div>
</div><!-- wrap-content -->
...
cu.layout.formActive();
	...
	$("#role-add-btn").click(function() {
	    if(checkForm("#role-form-add",'top')) {
			
			var list = $("#role-form-add").serialize();
			var menuIds = "";
			var permitIds = "";
			var pIds="";
			...
			$.post("role/addRole",reqData, function(json) {
				cu.toast.close();
	...
	$.post("role/getRoleComList",reqData,function(json){
		...
		if(res.result =="1"){
			...
			for(var i=0;i<roleCom.length;i++){
				str+='<tr><td><input class="roleComCheck" type="checkbox" onclick=getPermitZnodes_4(); value="'+roleCom[i][0]+'" style="cursor:pointer;" ></td><td><label style="cursor:pointer; color:#2392F7; text-decoration: underline;"  class="m-b-0 roleCom_text"  r_name="'+roleCom[i][1]+'" r_id="'+roleCom[i][0]+'"   >'+roleCom[i][1]+'</label></td></tr>';
			}
...
function getPermitZnodes_4(){
		var ids = getParentRoleIds();
		var reqData = "";
		if(ids.length<1)
			reqData = "&op=tree1";
		else
			reqData = "&op=tree4&panertRoleIds="+ids;
...
//组织机构树配置文件
	...
	function roleAddinitZnodes(){
		...
		$.post("org/orgZtree",reqData,function(json){
			...
			$.fn.zTree.init($("#ztree_role_add"), seeting_user_add, zNodes);	
		});	
	}
	...
	function showMenu() {
		...
		var formOffset =  $("#role-form-add").offset();
