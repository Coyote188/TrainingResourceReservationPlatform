...
<script type="text/javascript">
    ...
    $("#grid-list").jqGrid({
		url : "crud/queryPage?bean=roleList&flag=true&orgId=${userInfo.orgId}",
		datatype : 'json',
		mtype : 'POST',
		colNames : [ "角色ID", "角色名称","部门","角色备注", "操作" ],
		colModel : [ {name : 'roleId',index : 'id',key : true,hidden:true}, 
		             {name : 'roleName',width:'150px'}, 
		             {name : 'rogName',width:'220px'}, 
		             {name : 'roleDes'}, 
		             {name : 'content',width:'120px',align:'center'}
		            ],
		pager : '#pager',
		rowList : [ 10, 20, 30 ],
		rowNum : 20,
	...
	function onGridComplete() {
		...
		for ( var i = 0; i < ids.length; i++) {
			...
			var newName = '<a href="' + url2 + '" class="in-table role-info">';
			...
			html += '<span style="color:white ">配置用户</span></a>';
			html += '&nbsp;&nbsp;<a href="' + url3 + '"  class="a_btn btn btn-warning permit-role-config">';
			html += '<span style="color:white ">权限配置</span></a>';
			$("#grid-list").jqGrid('setRowData', ids[i], {
				content : html,roleName:newName
			});
		}
