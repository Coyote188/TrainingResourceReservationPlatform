		...
		<form id="form-user-add" >
			<table class="table table-hover table-condensed" >
				<tr>
					...
					<td >
					<input id="iptCardId" type="text" class="input30 require" length="1,32" label-name="卡号" name="objBean.id" value="${vCardId}"/>
					</td>
					<td class="td-col1">车牌号</td>
					<td>
					<input type="text" class="input30 require" length="1,6" label-name="车牌号" name="objBean.licensePlate" />
					</td>
				</tr>
				<tr>
					<td class="td-col1">车架号</td>
					<td >
					<input type="text" class="input30 require" length="1,32" label-name="车架号" name="objBean.frameNbr"/>
					</td>
					<td class="td-col1">车主</td>
					<td >
					<input type="text" class="input30 require" length="1,32" label-name="车主" name="objBean.owner"/>
					</td>
				</tr>
				<tr>
					<td class="td-col1">联系人</td>
					<td >
					<input type="text" class="input30 require" length="1,32" label-name="联系人" name="objBean.contacts"/>
					</td>
					<td class="td-col1">联系电话</td>
					<td >
					<input type="text" class="input30 require" length="1,32" label-name="联系电话" name="objBean.contactNbr"/>
					</td>
				</tr>
				<tr>
					<td class="td-col1">车辆所属公司</td>
					<td>
					  <select class="select30 " label-name="车辆所属公司" name="objBean.vehicleCompany" id="slt_company" >
					  </select>
					</td>
					<!--  
...
<script type="text/javascript">
	...
	function loadUserAddJs() {
		//
		if($.trim($("#iptCardId").val())!= null && $.trim($("#iptCardId").val()) != ""){
			$("#iptCardId").attr("disabled","disabled");
		}
		selectDictItem("#slt_company","CAR_BELONG","");
		...
		$("#add_user_btn").click(function() {
			if(checkForm("#form-user-add",'right')) {
				...
				$.post("vehicle/add?vCardId=${vCardId}",reqData, function(json) {
					cu.toast.close();
					var output = $.parseJSON(json.output);
					cu.toast(output.msg);
					if(output.result == '1') {
						BootstrapDialogUtil.closeCb();
					}
				});
			}
			return false;
