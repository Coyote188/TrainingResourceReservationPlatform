		...
		<form id="form-user-add" >
			<table class="table table-hover table-condensed" >
				<tr>
					...
					<td>
					<input type="text" class="input30 require" length="1,6" label-name="卡号" value="${vCardId}" name="objBean.vehicleId" />
					</td>
					<td class="td-col1">维修站</td>
					<td>
						<input disabled="disabled" id="orgSel" type="text" onclick="showMenu();" type="text" class="input30 require" style="width: 80%;" value="" readonly="">
						<input id="orgId" value="${objBean.orgId}" name= "objBean.orgId" value="" type="hidden" >					
					</td>
				</tr>
				<tr>
					<td class="td-col1">检测人</td>
					<td >
					<select class="select30 " label-name="检测人" name="objBean.inspector" id="slt_ins" ></select>
					</td>
					<td class="td-col1">例保时间</td>
					<td >
					<input type="text" id="ipt_maintime1" class="input30 require" length="1,32" label-name="例保时间" name="objBean.mtnTime"/>
					</td>
				</tr>
				<tr>
					<td class="td-col1">例保情况</td>
					<td colspan="3">
					 <textarea type="text" id="input3" class="form-control " style="width: 90%;" label-name="例保情况"  length="1,100" name="objBean.maintenance" />
					</td>
				</tr>
			</table>
...
<script type="text/javascript">
	...
	function loadUserAddJs() {
		...
		$.selectInspItem("#slt_ins","${orgId}","");
		userAddinitZnodes();
		cu.layout.formRequire();
		cu.layout.formActive();
		$("#ipt_maintime1").datepicker({ dateFormat: 'yy-mm-dd' });
		...
		$("#add_user_btn").click(function() {
			if(checkForm("#form-user-add",'right')) {
				...
				$.post("maintanence/add",reqData, function(json) {
					cu.toast.close();
