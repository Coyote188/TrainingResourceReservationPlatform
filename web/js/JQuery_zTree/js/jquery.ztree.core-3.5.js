...
(function($){
	...
	event = {
		...
		bindTree: function(setting) {
			...
			if(setting.myQuery.enable){
				setting.treeObj.append(setting.myQuery.showHtml);
			}
		},
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			$.ajax({
				...
				success: function(msg) {
					...
					var newNodes = [];
					var res = jQuery.parseJSON(msg.output);
					try {
						if (!res.data || res.data.length == 0) {
							newNodes = [];
						} else if (typeof res.data == "string") {
							newNodes = eval("(" + res.data  + ")");
						} else {
							newNodes = res.data ;
						}
					} catch(err) {
		...
		init: function(obj, zSetting, zNodes) {
			...
			var zTreeTools = {
				...
				reAsyncChildNodes : function(parentNode, reloadType, isSilent) {
					...
					//	alert(this.setting.data.key.children);
