	...
	$.fn.contextMenu = function(data, options) {
		...
		var htmlCreateMenu = function(datum) {
			...
			if ($.isArray(dataMenu) && dataMenu.length) {
				htmlMenu = '<div id="contextMenu_'+ nameMenu +'" class="'+ clKey +'box">' 
					+ '<div class="'+ clKey +'body">' + '<ul class="'+ clKey +'ul">';
			...
			return htmlMenu;
		}, funcontextMenu = function() {
			var idKey = "#contextMenu_", clKey = "smart_menu_", jqueryMenu = $(idKey + params.name);
			if (!jqueryMenu.size()) {
		...
		if (!B.data("bind")) {
			B.bind("click", $.contextMenu.hide).data("bind", true);
		}
	};
	$.extend($.contextMenu, {
		hide: function() {
