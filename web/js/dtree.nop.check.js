...
// Node object
function NOPNode(id, pid, name, url, title, target, isCheck, checkbox, icon,
		iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
...
// Tree object
function dNOPTree(objName, pathType) {
	this.config = {
		target : null,
		folderLinks : true,
		useSelection : true,
...
dNOPTree.prototype.add = function(id, pid, name, url, title, target, isCheck,
		checkbox, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new NOPNode(id, pid, name, url, title,
			target, isCheck, checkbox, icon, iconOpen, open);
};

// Open/close all nodes
dNOPTree.prototype.openAll = function() {
	this.oAll(true);
};
dNOPTree.prototype.closeAll = function() {
	this.oAll(false);
};

// Outputs the tree to the page
dNOPTree.prototype.toString = function() {
...
dNOPTree.prototype.addNode = function(pNode) {
	var str = '';
	var n = 0;
...
dNOPTree.prototype.node = function(node, nodeId) {
	var str = '<div class="dTreeNode">' + this.indent(node, nodeId);
	if (node.checkbox == true) {
		if (node.isCheck == null || node.isCheck == ""
				|| node.isCheck == "null" || node.isCheck == '-1') {
			str += '<input  name="ck" type="checkbox"  value="' + node.id
					+ '"  id="c' + this.obj + nodeId + '" onclick="javascript:'
					+ this.obj + '.cc(' + nodeId + ')"/>';
		} else {
			str += '<input  name="ck" checked ="checked" type="checkbox"  value="'
					+ node.id
					+ '"  id="c'
					+ this.obj
					+ nodeId
...
dNOPTree.prototype.indent = function(node, nodeId) {
	var str = '';
	if (this.root.id != node.pid) {
...
dNOPTree.prototype.setCS = function(node) {
	var lastId;
...
dNOPTree.prototype.getSelected = function() {
	var sn = this.getCookie('cs' + this.obj);
	return (sn) ? sn : null;
};

// Highlights the selected node
dNOPTree.prototype.s = function(id) {
	if (!this.config.useSelection)
		return;
...
dNOPTree.prototype.o = function(id) {
	var cn = this.aNodes[id];
...
dNOPTree.prototype.oAll = function(status) {
	for ( var n = 0; n < this.aNodes.length; n++) {
...
dNOPTree.prototype.openTo = function(nId, bSelect, bFirst) {
	if (!bFirst) {
		for ( var n = 0; n < this.aNodes.length; n++) {
...
dNOPTree.prototype.closeLevel = function(node) {
	for ( var n = 0; n < this.aNodes.length; n++) {
...
dNOPTree.prototype.closeAllChildren = function(node) {
	for ( var n = 0; n < this.aNodes.length; n++) {
...
dNOPTree.prototype.nodeStatus = function(status, id, bottom) {
...
dNOPTree.prototype.clearCookie = function() {
	var now = new Date();
...
dNOPTree.prototype.setCookie = function(cookieName, cookieValue, expires, path,
		domain, secure) {
...
dNOPTree.prototype.getCookie = function(cookieName) {
	var cookieValue = '';
...
dNOPTree.prototype.updateCookie = function() {
	var str = '';
	for ( var n = 0; n < this.aNodes.length; n++) {
...
dNOPTree.prototype.isOpen = function(id) {
...
dNOPTree.prototype.cc = function(nodeId) {
