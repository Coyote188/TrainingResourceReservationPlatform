...
// Node object
function Node(id, pid, name, url, title, target, isCheck, checkbox, icon,
		iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
...
// Tree object
function dTree(objName, pathType) {
	this.config = {
		target : null,
		folderLinks : true,
		useSelection : true,
...
dTree.prototype.add = function(id, pid, name, url, title, target, isCheck,
...
function choseAllChk(obj) {// 遍历所有取值
	...
	var allchx = document.getElementsByTagName("input");
	for ( var i = 0; i < allchx.length; i++) {
		if (allchx[i].checked == true)// allchx[i].name==obj.value)
		{// if
			// 取值
			alert(allchx[i].value);
		}
	}
}